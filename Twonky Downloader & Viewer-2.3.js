// ==UserScript==
// @name         Twonky Downloader & Viewer
// @version      2.31
// @description  Fix Twonky public Web UI and download all images
// @author       ltlwinston
// @match        http*://*/*
// @grant        GM_addElement
...
(async function () {
    ...
    function getAlbumName() {
        const albumNameElem = document.querySelector('.subheaderTitle');
        return albumNameElem ? albumNameElem.textContent.trim() : 'album';
    }

    async function fetchImagesFromAlbum(albumLink) {
        const response = await fetch(albumLink);
        const albumPageText = await response.text();
        const parser = new DOMParser();
        const doc = parser.parseFromString(albumPageText, 'text/html');
        const imageLinks = Array.from(doc.querySelectorAll('a[title="Download image"]')).map(a => a.href);
        return imageLinks;
    }
    ...
    if (document.title.match(/(twonky|pv connect|mediaserver)/i)) {
        ...
        // Function to download all images on the page excluding .gif files and create a ZIP file
        async function downloadAllImages() {
            const zip = new JSZip();
            const promises = [];
            const albumName = getAlbumName();
            const albumButtons = document.querySelectorAll('button#toalbum');

            if (albumButtons.length > 0) {
                for (const button of albumButtons) {
                    const albumLink = button.parentElement.querySelector('a').href;
                    const imageLinks = await fetchImagesFromAlbum(albumLink);
                ...
                document.querySelectorAll('a[title="Download image"]').forEach(function(a) {
                    const url = a.href;
                    const filename = getFilename(url);
                    promises.push(
                        fetch(url)
            ...
            Promise.all(promises).then(() => {
                zip.generateAsync({ type: "blob" }).then(content => {
                    saveAs(content, `${albumName}_images.zip`);
                });
            });
        }

        // Function to download all videos on the page and create a ZIP file
        async function downloadAllVideos() {
            ...
            document.querySelectorAll('a[title="Download video"]').forEach(function(a) {
                const url = a.href;
                const filename = getFilename(url);
                promises.push(
                    fetch(url)
            ...
            Promise.all(promises).then(() => {
                zip.generateAsync({ type: "blob" }).then(content => {
                    saveAs(content, `${albumName}_videos.zip`);
                });
            });
        }

        // Create and add the "Download All Images" button
        const downloadButton = document.createElement('button');
        downloadButton.id = 'downloadAllImages';
        downloadButton.innerText = 'Download All Images';
        downloadButton.style.position = 'fixed';
        downloadButton.style.bottom = '1em';
        downloadButton.style.right = '1em';
        ...
        // Create and add the "Download All Videos" button
        const downloadVideoButton = document.createElement('button');
        downloadVideoButton.id = 'downloadAllVideos';
        downloadVideoButton.innerText = 'Download All Videos';
        downloadVideoButton.style.position = 'fixed';
        downloadVideoButton.style.bottom = '1em';
        downloadVideoButton.style.right = '10em';
        downloadVideoButton.style.zIndex = '9999';
        downloadVideoButton.style.padding = '10px';
        downloadVideoButton.style.backgroundColor = '#4CAF50';
        ...
        document.body.appendChild(downloadVideoButton);

        downloadButton.addEventListener('click', downloadAllImages);
        downloadVideoButton.addEventListener('click', downloadAllVideos);
        ...
        if (USE_CACHE) {
            ...
            watchOnEvent('#wrapper', 'mouseenter', '.myLibraryMediaIconVideo img', async function (e) {
                ...
                if (info) {
                    ...
                    if (this.src && !aVid) {
                        ...
                        aVid.href = url;
                        aVid.download = getFilename(url);
                        aVid.title = 'Download video';
                        aVid.innerHTML = '<button style="font-size:0.8em;"><i class="fa fa-download"></i></button>';
            ...
            watchOnEvent('#wrapper', 'mouseenter', '.photoThumbnail', async function (e) {
                ...
                if (info) {
                    ...
                    if (this.src && !aImg && !this.src.endsWith('.gif')) {
                        aImg = document.createElement('a');
                        aImg.id = this.id + 'aimg';
                        aImg.href = this.src.replace(/\?.*/,'');
                        aImg.download = getFilename(this.src);
                        aImg.title = 'Download image';
                        aImg.innerHTML = '<button style="font-size:0.8em;"><i class="fa fa-download"></i></button>';
